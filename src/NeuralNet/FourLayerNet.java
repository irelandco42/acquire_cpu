...
public class FourLayerNet {
    ...
    public FourLayerNet(NeuronLayer layer1, NeuronLayer layer2, NeuronLayer layer3, NeuronLayer layer4) {
        this(layer1, layer2, layer3, layer4, 0.1);
    }

    public FourLayerNet(NeuronLayer layer1, NeuronLayer layer2, NeuronLayer layer3, NeuronLayer layer4, double learningRate) {
        this.layer1 = layer1;
        this.layer2 = layer2;
        this.layer3 = layer3;
        this.layer4 = layer4;
    ...
    public void think(double[][] inputs) {
        ...
        outputLayer4 = apply(matrixMultiply(outputLayer3, layer4.weights), layer4.activationFunction); // 4x1
    }
    ...
    public void train(double[][] inputs, double[][] outputs, int numberOfTrainingIterations) {
        for (int i = 0; i < numberOfTrainingIterations; ++i) {
            ...
            // calculate the error for layer 4
            // (the difference between the desired output and predicted output for each of the training inputs)
            double[][] errorLayer4 = matrixSubtract(outputs, outputLayer4); // 4x1
            double[][] deltaLayer4 = scalarMultiply(errorLayer4, apply(outputLayer4, layer4.activationFunctionDerivative)); // 4x1


            // calculate the error for layer 3
            // (the difference between the desired output and predicted output for each of the training inputs)
            double[][] errorLayer3 = matrixMultiply(deltaLayer4, matrixTranspose(layer4.weights)); // 4x1
            ...
            double[][] adjustmentLayer4 = matrixMultiply(matrixTranspose(outputLayer3), deltaLayer4); // 4x1
            ...
            adjustmentLayer4 = apply(adjustmentLayer4, (x) -> learningRate * x);
            ...
            this.layer4.adjustWeights(adjustmentLayer4);
        ...
        //total += 3;
        //think(new double[][] {testInputs[testInputs.length - 1]});
        //System.out.println("Correct " + total_correct + "/" + total);
        //System.out.println("Correct Percent " + ((double) total_correct / (double) total));
        System.out.println();
        System.out.println();
    }
        ...
        if (out != null) {
            ...
            for (int i = 0; i < layer4.weights.length; ++i) {
                for (int j = 0; j < layer4.weights[0].length; ++j) {
                    string = string + layer4.weights[i][j] + " ";
                }
                out.println(string);
                string = "";
            }
